...
function MobileMenuButton({
  ...
  return (
    ...
    >
      <span
        className={`bg-secondary-300 block h-1 w-6 rounded-2xl transition-transform duration-300 ${open ? 'translate-y-3 rotate-45' : ''}`}
      />
      <span
        className={`bg-secondary-300 block h-1 w-6 rounded-2xl transition-opacity duration-300 ${open ? 'opacity-0' : ''}`}
      />
      <span
        className={`bg-secondary-300 block h-1 w-6 rounded-2xl transition-transform duration-300 ${open ? '-translate-y-2 -rotate-45' : ''}`}
      />
    </button>
  );
}
